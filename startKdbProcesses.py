...
class kdbProc(object):
    ...
    def startProc(self):
        ...
        try: 
            self.proc = psutil.Popen(self.cmd, stdin=subprocess.DEVNULL, stdout=logFileHdl, stderr=logFileHdl)
            if self.checkAlive():
    ...
    def _queryProc(self, isSync, *args, **kwargs):
        ...
        try:
            with qconnection.QConnection(
                ...
                res = q.sendSync(*args,**kwargs) if isSync else q.sendAsync(*args,**kwargs)
        except Exception as e:
